...
저장고 원격제어 에서 제상 가동간격 (분)이 건조기 남은시간으로 표시됨
...
#### 전원 제어 개선
- **Power ON 시 재시작**: 
  Power OFF 상태에서 KEY_PWR로 Power ON 전환 시, Flash에 저장 후 시스템 재시작
- **정전 복구 개선**: 
  정전 후 전원 복귀 시 Flash에 저장된 이전 상태(ON/OFF) 유지
- **SmartConfig 중 전원 제어**: 
  SmartConfig 실행 중 KEY_PWR 누르면 Power ON 상태로 Flash 저장 후 재시작하여 SmartConfig 종료
